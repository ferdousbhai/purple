...
- **Sound palette**: the prebake in `packages/ui/src/use-strudel.ts` loads five sources — Dirt-Samples, tidal-drum-machines (`bank()`), piano, `gm_*` soundfonts, ZZFX `z_*` synths — plus `soundAlias` fixes (`ho→oh`, `cp→clap`, `rm→rim`). An unknown sound name plays SILENCE, not an error, so the repair loop can't catch it; the only defense is `STRUDEL_REFERENCE` in `packages/core/src/prompts.ts`, whose palette lists must stay in sync with the prebake. `PROMPT_EXAMPLES` are validated against the real engine by `packages/ui/src/strudel-examples.test.ts`, so never ship an example that test hasn't seen.
