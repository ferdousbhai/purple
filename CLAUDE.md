...
- **CodeMirror 6** - editable Strudel code editor (JavaScript + the shared violet theme in `@purple/ui`'s `editor-theme.ts`)
...
packages/ui/                       # @purple/ui - shared webview modules that need React/CodeMirror/
                                   ...
                                   # session-store (chat + working-pattern persistence),
...
- **Session persistence**: `@purple/ui/session-store` is the one place chat transcripts and the working editor pattern persist - both apps store them together in the webview's localStorage (Tauri's profile is persistent; Rust holds no session state), because restoring the transcript without the pattern it produced desyncs the session. The web app layers its BYOK rules on top in `apps/web/src/lib/byok.ts` (key-scoped storage keys, orphan purge when the key disappears, Riff legacy-key migration); the desktop uses the store's defaults in `usePurpleController`, where a restored session outranks the random no-args startup preset (`isFallback` on `StartupOptions`) but yields to explicit `-c`/`-p`/`--prompt` arguments.
