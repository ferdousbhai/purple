/**
 * The Strudel vocabulary Purple hands its agent. Distilled from the
 ...
 * error - Strudel fails open and plays silence, so only the studio's own
 * sound audit can catch it. The runtime also enforces an expression
 * allowlist and audits names before a pattern reaches the editor.
 */
...
- At most 2000 expression nodes and 100 levels of nesting; a long .a().b().c() chain counts as nesting, since each call wraps the one before. A refusal names which ceiling was hit.
