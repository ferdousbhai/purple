...
Most people do not need it: the website hosts an MCP relay, and the studio's
agent panel shows a registration command carrying the tab's private pairing
address. This bridge is the fully offline alternative for development or for
keeping even pattern code off the wire.

## Setup

Requires Node 22.18 or newer (the bin runs the TypeScript sources directly).

From a checkout, register the bridge with your agent:

```sh
claude mcp add purple -- node <repo>/packages/agent-bridge/bin/purple-mcp.mjs
codex mcp add purple -- node <repo>/packages/agent-bridge/bin/purple-mcp.mjs
```
...
The server also ships MCP `instructions` describing how a Purple set runs:
keep composing sections and crossfading into them until the listener stops.
